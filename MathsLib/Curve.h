...
namespace Curves
{
	...
	template <typename T>
	T CardinalSpline(const T* controlPoints, size_t count, float t , float Tension) {
		...
		T t0, t1;
		if (p0 == 0)
			t0 = (controlPoints[p0 + 1] - controlPoints[p0]) * Tension;
		else if (p0 == count - 1)
			t0 = (controlPoints[p0] - controlPoints[p0 - 1]) * Tension;
		else
			t0 = (controlPoints[p0 + 1] - controlPoints[p0 - 1]) * Tension;

		if (p1 == 0)
			t1 = (controlPoints[p1 + 1] - controlPoints[p1]) * Tension;
		else if (p1 == count - 1)
			t1 = (controlPoints[p1] - controlPoints[p1 - 1]) * Tension;
		else
			t1 = (controlPoints[p1 + 1] - controlPoints[p1 - 1]) * Tension;

		// how far are we through this segment
		float s = fmod(t, stepDuration) / stepDuration;

		// return curve result
		return hermite(controlPoints[p0], t0,
			controlPoints[p1], t1,
			s);
	}
}
